...
class TagifyUserListEntityAutocompleteMatcher extends TagifyEntityAutocompleteMatcher {
    ...
    if ($handler !== FALSE) {
      ...
      foreach ($entity_labels as $values) {
        foreach ($values as $entity_id => $label) {
          ...
          if ($label !== NULL) {
            $matches[$entity_id] = $this->buildTagifyUserListItem($entity_id, $label, $info_label, $selection_settings['image_style']);
          }
        }
      }
      if ($match_limit > 0) {
   ...
   * @throws \Drupal\Component\Plugin\Exception\PluginNotFoundException
   */
  protected function buildTagifyUserListItem(string $entity_id, string $label, ?string $info_label, string $image_style): array {
